...
const rssiRataElement = document.getElementById('rssi-rata');
const tablerssi = document.getElementById('table-rssi');
        ...
        .then(data => {
            ...
            newData.forEach(newItem => {
                const rowData = Object.entries(newItem)
                    .filter(([key]) => key == 'rssi') // Hanya ambil data dengan kunci 'rssi'
                    .map(([key, value]) => `${value} dBm`) // Format data 'rssi'
                    .join(', ');

                // Periksa apakah nilai rssi kurang dari -100
                if (parseInt(newItem.rssi) < -117) {
                    // Jika ya, tambahkan data ke elemen dengan id "rssi-limit"
                    const delayLimitElement = document.getElementById('rssi-limit');
                    if (delayLimitElement) {
                        ...
                        newDelayLimitElement.setAttribute('id', 'rssi-limit');
                ...
                // Hitung rata-rata dan perbarui elemen rssi-rata
                hitungRataRata();
...
// Fungsi untuk menghitung rata-rata RSSI dari data yang ada di tabel
function hitungRataRata() {
    const rows = document.querySelectorAll('#table-rssi tr');
    let total = 0;
    ...
    rows.forEach(row => {
        ...
        const rssi = parseFloat(rowData.split(' ')[0].trim()); // Ambil nilai RSSI dari data
    ...
    // Masukkan rata-rata ke elemen dengan id 'rssi-rata'
    if (rssiRataElement) {
        if (isNaN(rataRata)) {
            rssiRataElement.textContent = `---`;
        } else {
            rssiRataElement.textContent = `${rataRata.toFixed(2)} dBm`;
        }
    } else {
        console.error('Element with id "rssi-rata" not found.');
    }
}
